...
<form action="/bgm/mmsearch" method="post"
  ...
  </select><input type="name" id="select" name="select"  placeholder="검색 단어 입력">
...
<br> <br>
		...
		<form style="margin: 0px; padding: 0px; width: 1150px; " id="form1">
		<br>
			<h1>MovieChart</h1>
<br><br>
			<span class="sex"><span class="jender"><input
					type="radio" id="man" name="sex" value="0" onclick="checkSex()"
					checked><label id="manLb" for="man">현재상영작 </label></span><span
				class="jender"><input type="radio" id="woman" name="sex"
					value="1" onclick="location='/bgm/mexpect'"><label
			...
			<table class="movie">
				...
				<tr>
					...
					<%
					 /* Collection<MovieFileLink> values = movieMap.toArray();
						Iterator<MovieFileLink> iter2 = values.iterator(); */
					%>
					<%
						int i = 0;
						...
						for (i=0; i<movieMap.size(); i++) {
							...
							double reserve = movieLink.getMovieReserve(); 
							...
							if (i % 4 != 0 || i == 0) {
					%>
					<td class="td1" style=" color:black;" ><a
						...
						: <%=movieLink.getMovieDate()%><br>예매율 : <%=d.format(r)%>% 「<%=i+1 %>위」<br> <input type="button"
