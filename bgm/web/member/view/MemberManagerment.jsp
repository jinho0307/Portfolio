...
<script type="text/javascript">
	$(function() {
		...
		$('.link').click(function () {
			var id = $(this).text();
			window.open('/bgm/sone?userid='+id+'&where=1','회원정보 수정','width=900px, height=850px,channelmode=yes');
		});
	});
	function memberdel() {
...
<form action="/bgm/mseach" method="post">
<div align="center">
<a href="/bgm/sall">전체목록</a>
...
<div align="center" id="listDiv">
	<table align="center" id="memberList">
		<tr><th>아이디</th><th>이름</th><th>가입일</th><th>이메일</th><th>수신여부</th> <th>관리자 권한</th><th>탈퇴시키기</th></tr>
		<%for(Member n : list){ %>
		<tr><td align="center"><a href="#" class="link"><%=n.getUserId() %></a></td>
		<td align="center"><%= n.getUserName() %></td>
		<td align="center"><%=n.getEnroll_date() %></td>
		<td align="center"><%=n.getEmail() %></td>
		<td align="center"><%if(n.getReception().equals("on")){
			out.print("동의안함");} else{out.print("수신동의");}%></td>		
		<td align="center"><%if(n.getManager()==0){out.print("관리자");} else{out.print("일반회워");} %></td>
		<td align="center"><a href="/bgm/delmember1?userid=<%=n.getUserId()%>"><input type="button" value="탈퇴시키기"></a></td>
		<%} %>
		</tr>
	</table>
</div>
...
%>[이전] &nbsp;
<%}else{ %>
	<a href="/bgm/sall?page=<%= currentPage - 1%>">[이전]</a>&nbsp;
<%} %>
<% for(int p = startPage; p <= endPage; p++){ 
	if(p == currentPage){
%><b>[<%= p %>]</b>&nbsp;
<% }else{ %>
<a href="/bgm/sall?page=<%=p %>">[<%= p %>]</a>&nbsp;
<%}} %>
<% if(currentPage >= maxPage){ %>
[다음]&nbsp;
<%}else{ %>
<a href="/bgm/sall?=<%= currentPage + 1%>">[다음]</a>
<%}}else{ %> 
<!-- 검색결고가있을때 -->
<%
	if(currentPage <= 1){
%>[이전] &nbsp;
<%}else{ %>
	<a href="/bgm/mseach?page=<%= currentPage - 1%>&keyword1=<%=keyword%>&seachItem=<%=seach%>">[이전]</a>%nbsp;
<%} %>
<% for(int p = startPage; p <= endPage; p++){ 
	if(p == currentPage){
%><b>[<%= p %>]</b>&nbsp;
<% }else{ %>
<a href="/bgm/mseach?page=<%=p %>&keyword1=<%=keyword%>&seachItem=<%=seach%>">[<%= p %>]</a>&nbsp;
<%}} %>
<% if(currentPage >= maxPage){ %>
[다음]&nbsp;
<%}else{ %>
<a href="/bgm/mseach?page=<%= currentPage + 1%>&keyword1=<%=keyword%>&seachItem=<%=seach%>">[다음]</a>
<%}} %>
</div>
